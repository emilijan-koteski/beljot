...
- **Trump bidding branches by rule config, not variant name (D-VAR-1)** — the two variants both have TWO bidding rounds. **Bitola**: round 1 offers the public flipped candidate's suit, round 2 opens all suits, and four passes in round 2 reshuffles and rotates the dealer. **Croatian**: no candidate card at all (trump is a bare named suit, freely chosen in BOTH rounds), round 1 is bid on 6 visible cards with 2 still face-down, four passes reveals those 2 to their owner and opens round 2 on the full hand, and the dealer bidding last in round 2 is FORCED to pick — no reshuffle. Keep separate code paths per outcome (never a generic bidding soup), but select between them by reading a `VariantRules` config field — NEVER by comparing `state.Variant`. `bitola` and `croatia` are preset resolvers over that config; a future story exposes the individual rules at room creation. See Epic 12 / D-VAR-1
