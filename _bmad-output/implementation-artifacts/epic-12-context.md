...
2. **Trump candidate** — Croatian has none; trump is a bare named suit chosen freely, and the picker takes no card into hand. *(shipped)*
3. **Single bidding round** — Croatian bidding is ONE round on the six visible cards ("dalje" to pass); the two face-down cards stay hidden from everyone, their owner included, until trump is resolved, and only then merge into the hands (8 each). *(corrected 2026-08-22 — owner rule; supersedes the earlier "round-2 reveal" divergence, whose mid-bidding reveal and `event:face_down_revealed` are removed)*
4. **All-pass outcome** — the Croatian dealer speaks last (fourth) in the single round and *must* name a suit — "pod mus"; no round 2 and no reshuffle-and-rotate. *(corrected 2026-08-22 — the force fires after three passes in round 1)*
...
- A player's face-down cards never appear in another player's snapshot, and never in their own until trump is resolved (2026-08-22: the mid-bidding reveal no longer exists — the post-pick merge is the one reveal moment). Masking lives in the server-side snapshot builder; client-side masking is trivially defeated and breaks the server-authoritative requirement.
...
**D-VAR-1 (binding).** Variants are presets over a rule config, not branch conditions. Every divergence is a named field on a `VariantRules` struct resolved **once** at game initialization and carried on game state; `bitola` and `croatia` are preset resolvers each returning a *fully populated* config with no field left to a default. The engine reads config fields only — the variant string is never compared in bidding, declaration, scoring, or validation code. Separate code paths per rule outcome are still required; only the *selector* changes. The six fields (2026-08-22: `RevealFaceDownOnRound2` removed with the single-round correction): deal shape, trump candidate on/off, all-pass outcome, declaration overlap, declaration timing, tie rule. Five are read; `TieRule` is populated but deliberately unread, and stays that way.
...
- **Wire contract.** The round-2 reveal event (`event:face_down_revealed`) was the epic's one new event surface; it was retired on 2026-08-22 with the single-round correction — the post-pick snapshot alone delivers the merged 8-card hand. Any payload change needed to surface held-over points must update the TypeScript and Go event contract files in the *same commit*, and incurs the full drift gate: Go golden files, testdata, Zod schemas, contract tests. The client mirrors of the rule config and declaration logic are bound by the `testdata/contract` goldens.
...
- **Croatian bidding** renders a free four-suit choice with no candidate card through its single round; Bitola's candidate presentation and round-2 candidate-disable behaviour stay untouched. The face-down pair stays rendered as backs for everyone — the owner included — until trump is resolved.
