...
func TestProjectForSeat_CardPrivacy(t *testing.T) {
	scenarios := []struct {
		...
		{
			// Croatian round 2: 6 open cards plus 2 face-down per seat,
			// revealed to their owners (via the per-seat event, never here).
			name:  "croatian round 2 with face-down pairs revealed",
			build: func() *game.GameState { return testfixtures.NewGameCroatianMidBidding(4) },
		},
		{
