 ...
 *
 * French keeps its pre-existing asymmetry — `77` on the reds, `55` on the blacks.
 * That predates this story: a near-black glow at the red's strength reads as a
 * smudge under the parchment orb rather than a halo. Croatian's accents are all
 * mid-value, so one strength serves all four.
 *
 ...
 * Croatian lifts each accent to the same legibility on the same background —
 * leaves and acorns especially, since a mid-green and a mid-brown both vanish
 * into the table.
 *
...
export const SUIT_INK_ON_FELT: Record<CardDeck, Record<Suit, string>> = {
  ...
  croatian: { S: "#8fd07a", H: "#ff8585", D: "#e8c766", C: "#d69a63" },
};
